...
- categories: 用户指定的品类，必须从以下标准品类中选择：{category_enum}
  
  **映射逻辑**：将用户的具体衣物名称归纳到对应的标准品类
  示例：
    - "裤子/长裤/短裤/牛仔裤/休闲裤" → ["下装"]
    - "T 恤/衬衫/毛衣/卫衣/ Polo 衫" → ["上装"]
    - "裙子/连衣裙/半身裙/百褶裙" → ["裙装"]
    - "夹克/风衣/大衣/棉服/羽绒服" → ["外套"]
    - "运动鞋/皮鞋/靴子/凉鞋" → ["鞋履"]
    - "帽子/围巾/皮带/手表" → ["配饰"]
  ...
  注意：
    - 仅当用户明确要求某品类时提取，"今天穿什么"不提取品类
    - 如果用户说的衣物无法映射到标准品类，返回空数组
...
### 6. 场景/天气
- scene: 场景，必须从以下标准场景中选择：{scene_enum}
  
  **映射逻辑**：将用户的口语表达、隐喻、文化用语归纳到对应的标准场景
  示例：
    - "约会/相亲/见家长/桃花运/旺桃花/招桃花/脱单" → "约会"
    - "上班/办公/开会/面试/职场/通勤" → "面试" 或 "商务"
    - "出去玩/旅游/度假/放松/散心" → "旅行" 或 "度假"
    - "健身/跑步/打球/瑜伽/游泳/马拉松/爬山" → "运动"
    - "参加婚礼/当伴郎/当伴娘/喝喜酒/吃酒席" → "婚礼"
    - "在家/休息/宅家/居家/睡觉" → "居家"
    - "逛街/购物/买菜/遛弯/散步/随便穿" → "日常"
    - "露营/徒步/登山/探险/野餐" → "户外探险"
    - "年会/晚宴/派对/蹦迪/酒吧/KTV" → "派对"
    - "出差/出差几天/出差有会议" → "出差"
  
  注意：
    - 如果用户的表达无法映射到标准场景，返回 null
    - 不要强行匹配生僻或歧义表达
    - 优先选择最贴近的场景，而非字面匹配
