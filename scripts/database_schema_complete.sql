-- =====================================================
-- WuXing AI Stylist - Complete Database Schema
-- Version: 1.0 (完整合并版)
-- Description: 整合所有分散的表定义为单一完整文件
-- 包含: 表结构、索引、触发器、注释、初始数据
-- =====================================================
-- 使用说明：
-- 1. 此文件包含完整的数据库定义，可用于全新初始化
-- 2. Zeabur PostgreSQL 已预装 pgvector，无需 CREATE EXTENSION
-- 3. 本地开发环境需要执行 CREATE EXTENSION vector
-- 4. 执行顺序：扩展 → 表 → 索引 → 注释 → 初始数据 → 触发器
-- =====================================================

-- =====================================================
-- 0. 启用 pgvector 扩展（仅本地环境需要，Zeabur已预装）
-- =====================================================
...
CREATE TABLE IF NOT EXISTS items (
    ...
    embedding VECTOR,                              -- 语义向量 (BGE-M3)
    applicable_weather JSONB DEFAULT '[]',         -- 适用天气：["晴天", "雨天", "寒冷"]
    applicable_seasons JSONB DEFAULT '[]',         -- 适用季节：["春", "夏", "秋", "冬"]
    temperature_range JSONB DEFAULT '{}',          -- 适用温度范围：{"最低": int, "最高": int}
    functionality JSONB DEFAULT '{}',              -- 功能性：{"防水": bool, "透气": bool}
    thickness_level VARCHAR(20),                   -- 厚度等级：极薄/轻薄/适中/中厚/厚重
...
CREATE INDEX IF NOT EXISTS idx_wardrobe_embedding ON user_wardrobe 
    USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);
