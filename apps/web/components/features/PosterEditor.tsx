...
export const PosterEditor: React.FC<PosterEditorProps> = ({
  ...
  // 输入框通用样式 - 移动端优化
  const inputBaseStyle = "w-full px-3 md:px-4 py-3 rounded-xl border-2 transition-all duration-200 focus:outline-none focus:ring-0 text-[var(--brand-heading)] placeholder:text-[var(--brand-subtle)] text-base md:text-sm";
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="group">
        ...
        <div className="relative">
          ...
          />
          <div className="absolute right-3 bottom-3 text-xs font-medium text-[var(--brand-subtle)]">
            {quote.length}/200
          </div>
        </div>
      </div>
